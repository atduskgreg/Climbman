...
class Ragdoll extends FlxGroup
{
	...
	public function init()
	{
		...
		uTorso = new FlxNapeSprite(startX, startY);
		sprites.push(uTorso);
		lTorso = new FlxNapeSprite(startX, startY + 30);
		sprites.push(lTorso);

		rULeg = new FlxNapeSprite(startX + 20, startY + 100);
		sprites.push(rULeg);
		rLLeg = new FlxNapeSprite(startX + 20, startY + 150);
		sprites.push(rLLeg);
		lULeg = new FlxNapeSprite(startX - 20, startY + 100);
		sprites.push(lULeg);
		lLLeg = new FlxNapeSprite(startX - 20, startY + 150);
		sprites.push(lLLeg);

		rUArm = new FlxNapeSprite(startX + 20, startY);
		sprites.push(rUArm);
		rLArm = new FlxNapeSprite(startX + 20, startY);
		sprites.push(rLArm);
		lUArm = new FlxNapeSprite(startX - 20, startY);
		sprites.push(lUArm);
		lLArm = new FlxNapeSprite(startX - 20, startY);
		sprites.push(lLArm);

		head = new FlxNapeSprite(startX, startY - 40);
		sprites.push(head);

		ClimberController.instance.registerControllableLimb(lLArm);
		ClimberController.instance.registerControllableLimb(rLArm);
		ClimberController.instance.registerControllableLimb(rLLeg);
		ClimberController.instance.registerControllableLimb(lLLeg);
	...
	public function createGraphics(prefix:String)
	{
		...
		{
			return 'assets/images/$prefix/$part.png';
		}
		...
		lULeg.loadGraphic(getPath("ULeg"));
		lULeg.flipX = true;
		rLLeg.loadGraphic(getPath("LLeg"));
		lLLeg.loadGraphic(getPath("LLeg"));
		lLLeg.flipX = true;

		rUArm.loadGraphic(getPath("UArm"));
		lUArm.loadGraphic(getPath("UArm"));
		lUArm.flipX = true;
		rLArm.loadGraphic(getPath("LArm"));
		lLArm.loadGraphic(getPath("LArm"));
		lLArm.flipX = true;
	}
	...
	function createJoints()
	{
		...
		constrain = new PivotJoint(lUArm.body, uTorso.body, new Vec2(uarmSize.x / 2 - 3, -uarmSize.y / 2 + 3),
			new Vec2(-uTorsoSize.x / 2 + 3, -uTorsoSize.y / 2 + 3));
		constrain.space = FlxNapeSpace.space;
		constrain = new PivotJoint(rUArm.body, uTorso.body, new Vec2(-uarmSize.x / 2 + 3, -uarmSize.y / 2 + 3),
			new Vec2(uTorsoSize.x / 2 - 3, -uTorsoSize.y / 2 + 3));
		constrain.space = FlxNapeSpace.space;
